...
| `claude/settings.json` | Modelo, estilo de salida, permisos y **los hooks**. En el `allow`, desde el 18-09-2026, el bloque de **leer sin preguntar** —el proyecto entero, la documentación de Claude Code y las órdenes de shell que solo miran—: preguntar por cada lectura convierte una investigación de tres minutos en veinte de clics |
| `.claude/settings.json` | El mismo bloque de lectura, **dentro** del repositorio. El de usuario no viaja: una sesión en la nube solo tiene lo que está commiteado, así que ahí el único `allow` que existe es este |
...
| `claude/bin/revision-pendiente.sh` | Dice **qué queda por leer**: `nada`, `todo`, o el SHA desde el que basta con revisar. Seguir trabajando en una rama ya revisada dejó de costar releerla entera —en erp, el 53 % de lo que leía la segunda pasada no había cambiado desde la primera—. **No relaja ningún freno** y no rebaja el nivel; cae a `todo` con cualquier duda: sin marca, marca sin nivel, marca que un rebase dejó fuera de la historia, o un diff que ahora pide más nivel |
...
| `claude/skills/` | `rama`, `probar`, `cerrar` (el flujo de ramas), más `auditar` (el repo entero, doce dimensiones, un informe y ningún arreglo), `despliega`, `grill-me` y `new-project`, esta última apagada en `settings.json` (`skillOverrides`) y aquí solo para volver a encenderla. **Las tres del flujo —`grill`, `revisar-antes-de-terminar` y `commits-a-mi-nombre`— ya no están aquí** *(18-09-2026)*: viven en las skills de la cuenta de claude.ai, que es el único sitio desde el que llegan a todos los proyectos **y** a las sesiones en la nube. Estuvieron aquí y en `erp` doce horas, las que tardó en subirlas, y en esas doce horas dos de las tres ya se habían desfasado respecto a su original — que es exactamente por lo que se acordó borrarlas el día que se escribieron. No se vuelven a copiar: si falta una, se sube a la cuenta |
| `verificar.sh` | Sintaxis de todos los guiones, el bit de ejecución con el que se instalan, **que los dos `settings.json` sean JSON** —una coma de más no la ve nadie hasta que Claude Code arranca sin modelo, sin estilo y sin los cuatro hooks—, el presupuesto de tamaño del `CLAUDE.md`, y las cinco matrices **solo si el cambio toca `claude/bin` o `claude/hooks`**: un freno de antes del push tiene que costar segundos, y el CI entero de este repo tarda 32 |
