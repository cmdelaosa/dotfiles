...
| `claude/skills/` | `rama`, `probar`, `cerrar` (el flujo de ramas), más `auditar` (el repo entero, doce dimensiones, un informe y ningún arreglo), `despliega`, `grill-me` y `new-project`, esta última apagada en `settings.json` (`skillOverrides`) y aquí solo para volver a encenderla. Y las tres del flujo que tienen que valer **también en una sesión en la nube** —`grill`, `revisar-antes-de-terminar` y `commits-a-mi-nombre`—, escritas genéricas a propósito: no nombran proyecto, ni `verificar.sh`, ni `PLAN.md`. Su copia canónica es la de `.claude/skills/` de `erp` (PR #233); esta es la que las hace valer en todos los proyectos locales, y ambas caducan el día que estén subidas a las skills de la cuenta de claude.ai, que es el único sitio desde el que llegan a la nube |
