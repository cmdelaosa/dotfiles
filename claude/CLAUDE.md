...
- **The local stack does not come up on its own — ask me first** *(2026-08-14)*. Bringing one up builds images, clones my data volume and takes a port, and most branches I judge entirely from the PR. So `probar-rama.sh` raises nothing by default; `--con-pila` raises it in the same run, `--solo-pila` raises it later on its own. **Ask me before anything else starts**, so that my answer and the two brakes above and the CI all burn the same twelve minutes instead of taking them in turn — and so the run can be launched with the answer already in it. The `probar` skill has the procedure; this line has the rule, so that when the two disagree it is the skill that is wrong.
