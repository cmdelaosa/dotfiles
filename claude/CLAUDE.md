...
- **Closing has two halves, and the second one is mine to trigger.** First `~/.claude/bin/probar-rama.sh <branch>`: it pushes, opens the PR, waits for CI and stops there with the PR's URL. It **never merges**. Run it in the background; welzy's CI takes ~12 minutes. Then I look at it. If something needs changing, change it in the branch and run it again — that is the point: `main` gets one clean merge instead of three commits fixing what testing revealed.
- **The local stack does not come up on its own — ask me first** *(2026-08-14)*. Bringing one up builds images, clones my data volume and takes a port, and most branches I judge entirely from the PR. So `probar-rama.sh` raises nothing by default; `--con-pila` raises it in the same run and `--solo-pila` raises it later on its own, skipping the push, the PR and the CI wait. **Ask at the very start and don't wait for my answer**: launch `probar-rama.sh` in the background first, then ask with `AskUserQuestion` while CI, `verificar.sh` and the reviewer are already running — an answer that costs twelve minutes of waiting is an answer nobody asks for. If I say yes, run `--solo-pila` once the background run comes back green. Skip the question entirely when the worktree has no `docker-compose.yml`: there is no stack to raise, and in `dotfiles` there never is.
