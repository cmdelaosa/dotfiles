...
- **Two things happen before the push, and the skill `probar` is where they live** *(2026-08-13)*:
  - **`verificar.sh` at the repo root** — format, lint, typecheck, fast unit tests. `probar-rama.sh` runs the **worktree's** copy and refuses to push if it fails. Discovering a lint error in CI costs the whole 12-minute round trip; here it costs seconds. If a repo has no `verificar.sh`, it says so and carries on — say it too when you report, because then CI is the only net. `--sin-verificar` skips it and is mine to authorise.
